...
Matrix& Matrix::Copy(const Matrix &A)
{
	...
	SetDim(A.row,A.col);
	for(register size_t n=0; n<size(); n++) (*this)[n] = A[n];
	return *this;
}
...
{
	Matrix T(row,col);
	for(register size_t n=0; n<size(); n++) T[n] = (*this)[n] + B[n];
	return T;
}

Matrix& Matrix::operator+= (const Matrix &B)
{
	for(register size_t n=0; n<size(); n++) (*this)[n] -= B[n];
	return *this;
}
...
{
	Matrix T(row,col);
	for(register size_t n=0; n<size(); n++) T[n] = (*this)[n] - a;
	return T;
}
...
{
	Matrix T(row,col);
	for(register size_t n=0; n<size(); n++) T[n] = (*this)[n] * val;
	return T;
}

Matrix& Matrix::operator*= (const double val)
{
	for(register size_t n=0; n<size(); n++) (*this)[n] *= val;
	return *this;
}
...
{
	Matrix R(row,col);
	for(register size_t n=0; n<size(); n++) R[n] = sqrt((*this)[n]);
	return R;
}
...
{
	T.SetDim(row,col);
	for(register size_t n=0; n<size(); n++) T[n] = (*this)[n] * A[n];
...
{
	double val = 0.0;
	for(register size_t n=0; n<size(); n++) val += (*this)[n] * A[n];
	return val;
}
